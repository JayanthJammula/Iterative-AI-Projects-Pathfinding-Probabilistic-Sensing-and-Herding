...
def initialize_knowledge_grid(ship, D, open_cells, sensor_cells,bot_pos):
    ...
    crew_knowledge = [[1/len(open_cells)-1 if ship[i][j] not in  ('B','0',bot_pos) else 0 for j in range(D)] for i in range(D)]
...
def main():
    ...
    x= bot3_run(ship,goal,S,D,k,alpha, aliens_pos,open_cells,crew_knowledge,alien_knowledge)
...
def bot3_run(ship,crew_pos,bot_pos,D,k,alpha,alien_pos,open_cells,crew_knowledge,alien_knowledge):
    ...
    while step<1000:
        ...
        #Sense Crew 
        if(len(crew_pos)>=1):
            crew_sense=sense_crew1(ship,bot_pos,crew_pos,alpha)
        else:
           return "SUCCESS",step
