...
def main():
    ...
    x= bot1_run(ship,goal,S,D,k,alpha, aliens_pos,open_cells,crew_knowledge,alien_knowledge)

    print(x)


def bot1_run(ship,crew_pos,bot_pos,D,k,alpha,alien_pos,open_cells,crew_knowledge,alien_knowledge):
    
    step=1
    path=None
    final_bot_path = [bot_pos]
    while step<1000:
        ...
        if path and len(path)>=1:
            ...
            ship[x][y] = '1' 
            ship[x_new][y_new] = 'B'
            final_bot_path.append((x_new,y_new))                
            print("\n")
