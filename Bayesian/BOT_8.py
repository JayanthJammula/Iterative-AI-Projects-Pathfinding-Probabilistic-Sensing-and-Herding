...
def update_alien_knowledge_before_movement(alien_knowledge,alien_knowledge_updated,sense,sensor_cells,D):    
    ...
    for i in range(D):
        for j in range(D):
            if alien_knowledge_updated[i][j]!=0:
                sum=0
                for x in range(D):
                    for y in range(D):
                        if alien_knowledge[i][j][x][y]!=0:
                            if (i,j) in sensor_cells:
                                prob1=1
                            else:
                                prob1=0
                            ...
                            if sense==False:
                                ...
                                prob= prob1+prob2-(prob1*prob2)                        
                            alien_knowledge[i][j][x][y]= (alien_knowledge[i][j][x][y]*prob)/denom
                            sum =sum+alien_knowledge[i][j][x][y]
                alien_knowledge_updated[i][j]=sum
...
def get_two_max_values_from_crew_knowledge(crew_knowledge_updated):
    # Minimize the crew_knowledge into a 1D list
    Minimized_list = [(i, j, crew_knowledge_updated[i][j]) for i in range(len(crew_knowledge_updated)) for j in range(len(crew_knowledge_updated[0]))]
    # Get the two maximum values from the flattened list
    sorted_lst = sorted(Minimized_list, key=lambda x: x[2], reverse=True)
...
def main():
    ...
    x= bot8_run(ship,goal,S,D,k,alpha, aliens_pos,open_cells,crew_knowledge,crew_knowledge_updated,alien_knowledge,alien_knowledge_updated)
...
def bot8_run(ship,crew_pos,bot_pos,D,k,alpha,alien_pos,open_cells,crew_knowledge,crew_knowledge_updated,alien_knowledge,alien_knowledge_updated):
    ...
    while step<1000:
        ...
        if(len(crew_pos)>=1):
            ...
            print("Sensed-Crew-2",crew_sense)
        ...
        max_crew_indices = get_two_max_values_from_crew_knowledge(crew_knowledge_updated)

        d1=heuristic_Bot_1(bot_pos,max_crew_indices[0])
        d2=heuristic_Bot_1(bot_pos,max_crew_indices[1])
        if(d1 < d2):
            c_x,c_y = max_crew_indices[0]
        else:
            c_x,c_y = max_crew_indices[1]  
        max_crew_pos=c_x,c_y
        for i in range(D):
            for j in range(D):
        ...
        #Finding Path
        path= astar(temp_ship,bot_pos,max_crew_pos,D,alien_knowledge_updated,crew_knowledge_updated,k)
        if path and len(path)>1:
            ...
            if status=="NEXT":
                ...
                #updating alien knowledge after the alien moves and normalizing it
