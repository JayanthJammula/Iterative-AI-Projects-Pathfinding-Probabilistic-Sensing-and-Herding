...
def get_dead_ends(ship, D):
    result = []
    #Checking for open cells with only one neighbour
    for i in range(D):
        for j in range(D):
            if ship[i][j] == '1':
                neighbours = get_open_neighbours_count(ship, D, i, j)
                if neighbours == 1:
                    result.append((i, j))
    return result
def heuristic(ship,D,a,b,alien_knowledge, crew_knowledge,k):
    heuristic = 0
    #Finding distance between bot and max belief alien cell
    manhattan_dist = abs(a[0] - b[0]) + abs(a[1] - b[1])
    
    crew_know = calculate_density(ship,D,a,crew_knowledge,k)
    alien_know = alien_knowledge[a[0]][a[1]]
    #adding utility
    heuristic = 0.6 * manhattan_dist + 0.6*alien_know - 0.8 * crew_know

    return heuristic

def calculate_density(ship, D,a,crew_knowledge,k):
    region = get_sensor_cells(a, k,D)
    sum = 0
    for cell in region:
        sum += crew_knowledge[cell[0]][cell[1]]
    return sum

def astar(ship, S, G, D,alien_knowledge,crew_knowledge,k):
    ...
    while not fringe.empty():
        ...
        for n in neighbours:
            ...
            if n not in dist or temp_dist < dist[n]:
                ...
                priority = temp_dist + heuristic(ship,D,n, G,alien_knowledge,crew_knowledge,k)
...
def initialize_knowledge_grid(ship, D, open_cells, sensor_cells):
    #initilizing the initial values for crew and alien knowledge grid
    crew_knowledge = [[0 for j in range(D)] for i in range(D)]
    alien_knowledge = [[0 for j in range(D)] for i in range(D)]
    for i in range(D):
        for j in range(D):
            if ship[i][j] not in ('B', '0'):  # Open cell
                crew_knowledge[i][j] = float(1 / len(open_cells)-1) 
            else:
                crew_knowledge[i][j] = 0  

            if (i, j) not in sensor_cells and ship[i][j] not in ('B', '0'):
                if ship[i][j] == '1':
                    alien_knowledge[i][j] = float(1 / (len(open_cells)) - len(sensor_cells) -1 )
...
def update_crew_knowledge_before_movement(ship,crew_knowledge,sense,D,bot_pos,alpha):
    ...
    for i in range(D):
        for j in range(D):
            ...
            prob=exp(-alpha * (d-1))
            if sense:
                # If there is a beep in cell, divide crew knowledge by prob
    ...
    if sense:
        for i in range(D):
            for j in range(D):
                ...
                if ship[i][j] not in ('B','0'):
                    # If the cell an open cell and there is beep, divide crew knowledge by denom
        ...
        for i in range(D):
            for j in range(D):
                ...
                if ship[i][j] not in ('B','0'):
                    # If the cell an open cell and there is no beep, divide crew knowledge by denom
...
def sense_crew(ship, bot_pos, crew_pos, alpha):
    x,y=bot_pos
    i,j=crew_pos
    #calculating  distance between bot's position and crew_pos
    d = abs(i-x) + abs(j-y)
...
def main():
    ...
    crew_knowledge,alien_knowledge=initialize_knowledge_grid(ship,D,open_cells,sensor_cells)


    x= bot2_run(ship,goal,S,D,k,alpha, aliens_pos,open_cells,crew_knowledge,alien_knowledge)

    print(x)

def get_two_max_values_from_crew_knowledge(crew_knowledge):
    # Flatten the matrix into a 1D list
    flattened_list = [(i, j, crew_knowledge[i][j]) for i in range(len(crew_knowledge)) for j in range(len(crew_knowledge[0]))]
    # Get the two maximum values from the flattened list
    sorted_lst = sorted(flattened_list, key=lambda x: x[2], reverse=True)
    max_values = sorted_lst[:2]
    # Extract the indices of the maximum values
    max = [(i, j) for i, j, _ in max_values]
    return max
...
def bot2_run(ship,crew_pos,bot_pos,D,k,alpha,alien_pos,open_cells,crew_knowledge,alien_knowledge):
    
    step=1
    path=None
    while step<1000:
        #getting sensor cells
        sensor_cells=get_sensor_cells(bot_pos,k,D)
        print("-------------------------\n Step ",step,"\n\n")

        #Sense Crew 
        crew_sense=sense_crew(ship,bot_pos,crew_pos,alpha)
        ...
        #Sense Aliens
        alien_sense=sense_aliens(ship,bot_pos,D,k,sensor_cells)
        ...
        #getting 2 crew positions with max probabilities 
        max_crew = get_two_max_values_from_crew_knowledge(crew_knowledge)
        d1=heuristic_Bot_1(bot_pos,max_crew[0])
        d2=heuristic_Bot_1(bot_pos,max_crew[1])
        if(d1 < d2):
            c_x,c_y = max_crew[0]
        else:
            c_x,c_y = max_crew[1]
        ...
        #Finding path
        path= astar(temp_ship,bot_pos,(c_x,c_y),D,alien_knowledge,crew_knowledge,k)
