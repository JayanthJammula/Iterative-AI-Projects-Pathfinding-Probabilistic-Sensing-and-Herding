            ...
            if crew_knowledge_updated[i][j]!=0:
                for x in range(D):
                    for y in range(D):
                        ...
                        #calculating  distance between bot's position and crew_pos-1
                            d1=abs(bpx-i)+abs(bpy-j)
                            prob1=exp(-alpha * (d1-1))
                            prob2=0
                            
                            if len(crew_pos)>1:
                        #calculating  distance between bot's position and crew_pos-2
...
def main():
    ...
    x= bot5_run(ship,goal,S,D,k,alpha, aliens_pos,open_cells,crew_knowledge,crew_knowledge_updated,alien_knowledge)
...
def bot5_run(ship,crew_pos,bot_pos,D,k,alpha,alien_pos,open_cells,crew_knowledge,crew_knowledge_updated,alien_knowledge):
    ...
    while step<1000:
        ...
        temp_ship[a_x][a_y]='A'
         #find path
        path= astar(temp_ship,bot_pos,max_crew_pos,D,alien_knowledge,crew_knowledge_updated,k)
