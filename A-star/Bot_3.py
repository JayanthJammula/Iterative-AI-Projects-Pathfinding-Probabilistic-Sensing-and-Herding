...
def bot3sim(ship, S, G, aliens_pos, D):
    status = False
    attempts = 0
    max_attempts = 1000
    ...
    while attempts<max_attempts:
        ...
        temp_ship = [row[:] for row in ship]  # Creating a copy of the ship
        neighbours=[]
        ...
        for alien in aliens_pos:
            ...
            for neighbour in neighbours:
                x, y = neighbour
                if 0 <= x < D and 0 <= y < D:
                    temp_ship[x][y] = '0'  # Mark the neighbor as blocked
        
        # Find path using A* algorithm considering the temporary ship representation
        ...
        if path and len(path)>=1:
            ...
            #If Bot Saves Captain
            elif ((x_new, y_new) == G and (x_new, y_new) not in aliens_pos):
                print("Success")
                print_ship(ship,D)
                visualize_ship(ship, G, D)
                plt.pause(0.8)
        ...
        # Update the ship and aliens_pos as before
        aliens_pos=movealiens(ship, aliens_pos, D)

        #If Aliens enter the bot cell
        if (S in aliens_pos):
            print("Failure!!")
            print_ship(ship,D)
            visualize_ship(ship, G, D)
